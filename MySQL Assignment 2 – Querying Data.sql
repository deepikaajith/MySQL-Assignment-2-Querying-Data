...
select sum(salary) from employees as Total_Finance_salary
...
select location_id, max(salary) from employees as Maximum_Salary
...
select location_id, avg(age) as Average_age from employees
where Gender = 'F'
group by location_id
having avg (age) < 30 ;
...
select d.department_id, count(e.employee_id) as Total_employee from departments d
left join employees e on d.department_id = e.department_id
Group by d.department_id;

select l.location, e.employee_name from employees e right join location l
